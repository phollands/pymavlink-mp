						       ...
						       uint8_t type, uint8_t color, uint8_t coordinate_system, uint16_t timeout, float xp1, float yp1, float zp1, float xp2, float yp2, float zp2, const char *name)
{
	...
	put_float_by_index(xp1, 0,  MAVLINK_PAYLOAD(msg)); // X1 Position
	put_float_by_index(yp1, 4,  MAVLINK_PAYLOAD(msg)); // Y1 Position
	put_float_by_index(zp1, 8,  MAVLINK_PAYLOAD(msg)); // Z1 Position
	put_float_by_index(xp2, 12,  MAVLINK_PAYLOAD(msg)); // X2 Position
	put_float_by_index(yp2, 16,  MAVLINK_PAYLOAD(msg)); // Y2 Position
	put_float_by_index(zp2, 20,  MAVLINK_PAYLOAD(msg)); // Z2 Position
	put_uint16_t_by_index(timeout, 24,  MAVLINK_PAYLOAD(msg)); // 0: no timeout, >1: timeout in seconds
	put_uint8_t_by_index(type, 26,  MAVLINK_PAYLOAD(msg)); // 0: Notice, 1: Warning, 2: Critical, 3: Emergency, 4: Debug
	put_uint8_t_by_index(color, 27,  MAVLINK_PAYLOAD(msg)); // 0: blue, 1: yellow, 2: red, 3: orange, 4: green, 5: magenta
	put_uint8_t_by_index(coordinate_system, 28,  MAVLINK_PAYLOAD(msg)); // 0: global, 1:local
