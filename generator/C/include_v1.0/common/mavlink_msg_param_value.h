						       ...
						       const int8_t param_id[15], float param_value, uint16_t param_count, uint16_t param_index)
{
	...
	put_int8_t_array_by_index(param_id, 8, 15,  msg->payload); // Onboard parameter id

	return mavlink_finalize_message(msg, system_id, component_id, 23, 17);
}
						           ...
						           const int8_t param_id[15],float param_value,uint16_t param_count,uint16_t param_index)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 23, 17);
}
						           ...
						           const int8_t param_id[15],float param_value,uint16_t param_count,uint16_t param_index)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 23, 17);
}
...
{
	return MAVLINK_MSG_RETURN_int8_t_array(msg, param_id, 15,  8);
}
