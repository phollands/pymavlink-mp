						       ...
						       uint8_t mode, uint8_t nav_mode, uint8_t status, uint16_t load, uint16_t vbat, uint16_t battery_remaining, uint16_t packet_drop)
{
	...
	put_uint16_t_by_index(load, 0,  msg->payload); // Maximum usage in percent of the mainloop time, (0%: 0, 100%: 1000) should be always below 1000
	put_uint16_t_by_index(vbat, 2,  msg->payload); // Battery voltage, in millivolts (1 = 1 millivolt)
	put_uint16_t_by_index(battery_remaining, 4,  msg->payload); // Remaining battery energy: (0%: 0, 100%: 1000)
	put_uint16_t_by_index(packet_drop, 6,  msg->payload); // Dropped packets (packets that were corrupted on reception on the MAV)
	put_uint8_t_by_index(mode, 8,  msg->payload); // System mode, see MAV_MODE ENUM in mavlink/include/mavlink_types.h
	put_uint8_t_by_index(nav_mode, 9,  msg->payload); // Navigation mode, see MAV_NAV_MODE ENUM
	put_uint8_t_by_index(status, 10,  msg->payload); // System status flag, see MAV_STATUS ENUM

	return mavlink_finalize_message(msg, system_id, component_id, 11, 95);
}
						           ...
						           uint8_t mode,uint8_t nav_mode,uint8_t status,uint16_t load,uint16_t vbat,uint16_t battery_remaining,uint16_t packet_drop)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 11, 95);
}
						           ...
						           uint8_t mode,uint8_t nav_mode,uint8_t status,uint16_t load,uint16_t vbat,uint16_t battery_remaining,uint16_t packet_drop)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 11, 95);
}
