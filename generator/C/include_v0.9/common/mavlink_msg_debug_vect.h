						       ...
						       const char *name, uint64_t usec, float x, float y, float z)
{
	...
	put_char_array_by_index(name, 0, 10,  MAVLINK_PAYLOAD(msg)); // Name
	put_uint64_t_by_index(usec, 10,  MAVLINK_PAYLOAD(msg)); // Timestamp
	put_float_by_index(x, 18,  MAVLINK_PAYLOAD(msg)); // x
	put_float_by_index(y, 22,  MAVLINK_PAYLOAD(msg)); // y
	put_float_by_index(z, 26,  MAVLINK_PAYLOAD(msg)); // z

	return mavlink_finalize_message(msg, system_id, component_id, 30, 239);
}
						           ...
						           const char *name,uint64_t usec,float x,float y,float z)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 30, 239);
}
						           ...
						           const char *name,uint64_t usec,float x,float y,float z)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 30, 239);
}
...
{
	return MAVLINK_MSG_RETURN_uint64_t(msg,  10);
}
