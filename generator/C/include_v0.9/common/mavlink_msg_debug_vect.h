						       ...
						       const char name[10], uint64_t usec, float x, float y, float z)
{
	...
	put_char_array_by_index(name, 0, 10,  msg->payload); // Name
	put_uint64_t_by_index(usec, 10,  msg->payload); // Timestamp
	put_float_by_index(x, 18,  msg->payload); // x
	put_float_by_index(y, 22,  msg->payload); // y
	put_float_by_index(z, 26,  msg->payload); // z
