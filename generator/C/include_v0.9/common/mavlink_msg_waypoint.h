						       ...
						       uint8_t target_system, uint8_t target_component, uint16_t seq, uint8_t frame, uint8_t command, uint8_t current, uint8_t autocontinue, float param1, float param2, float param3, float param4, float x, float y, float z)
{
	...
	put_uint8_t_by_index(frame, 4,  msg->payload); // The coordinate system of the waypoint. see MAV_FRAME in mavlink_types.h
	put_uint8_t_by_index(command, 5,  msg->payload); // The scheduled action for the waypoint. see MAV_COMMAND in common.xml MAVLink specs
	put_uint8_t_by_index(current, 6,  msg->payload); // false:0, true:1
	put_uint8_t_by_index(autocontinue, 7,  msg->payload); // autocontinue to next wp
	put_float_by_index(param1, 8,  msg->payload); // PARAM1 / For NAV command waypoints: Radius in which the waypoint is accepted as reached, in meters
	put_float_by_index(param2, 12,  msg->payload); // PARAM2 / For NAV command waypoints: Time that the MAV should stay inside the PARAM1 radius before advancing, in milliseconds
	put_float_by_index(param3, 16,  msg->payload); // PARAM3 / For LOITER command waypoints: Orbit to circle around the waypoint, in meters. If positive the orbit direction should be clockwise, if negative the orbit direction should be counter-clockwise.
	put_float_by_index(param4, 20,  msg->payload); // PARAM4 / For NAV and LOITER command waypoints: Yaw orientation in degrees, [0..360] 0 = NORTH
	put_float_by_index(x, 24,  msg->payload); // PARAM5 / local: x position, global: latitude
	put_float_by_index(y, 28,  msg->payload); // PARAM6 / y position: global: longitude
	put_float_by_index(z, 32,  msg->payload); // PARAM7 / z position: global: altitude
