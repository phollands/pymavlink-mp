						       ...
						       uint64_t usec, uint8_t fix_type, int32_t lat, int32_t lon, int32_t alt, float eph, float epv, float v, float hdg)
{
	...
	put_uint8_t_by_index(fix_type, 8,  msg->payload); // 0-1: no fix, 2: 2D fix, 3: 3D fix. Some applications will not use the value of this field unless it is at least two, so always correctly fill in the fix.
	put_int32_t_by_index(lat, 9,  msg->payload); // Latitude in 1E7 degrees
	put_int32_t_by_index(lon, 13,  msg->payload); // Longitude in 1E7 degrees
	put_int32_t_by_index(alt, 17,  msg->payload); // Altitude in 1E3 meters (millimeters)
	put_float_by_index(eph, 21,  msg->payload); // GPS HDOP
	put_float_by_index(epv, 25,  msg->payload); // GPS VDOP
	put_float_by_index(v, 29,  msg->payload); // GPS ground speed (m/s)
	put_float_by_index(hdg, 33,  msg->payload); // Compass heading in degrees, 0..360 degrees
...
#else
	memcpy(gps_raw_int, msg->payload, 37);
#endif
}
