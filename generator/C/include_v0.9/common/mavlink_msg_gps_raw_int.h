						       ...
						       uint64_t usec, uint8_t fix_type, int32_t lat, int32_t lon, int32_t alt, float eph, float epv, float v, float hdg)
{
	...
	put_int32_t_by_index(msg, 9, lat); // Latitude in 1E7 degrees
	put_int32_t_by_index(msg, 13, lon); // Longitude in 1E7 degrees
	put_int32_t_by_index(msg, 17, alt); // Altitude in 1E3 meters (millimeters)
	put_float_by_index(msg, 21, eph); // GPS HDOP
	put_float_by_index(msg, 25, epv); // GPS VDOP
	put_float_by_index(msg, 29, v); // GPS ground speed (m/s)
	put_float_by_index(msg, 33, hdg); // Compass heading in degrees, 0..360 degrees

	return mavlink_finalize_message(msg, system_id, component_id, 37, 149);
}
						           ...
						           uint64_t usec,uint8_t fix_type,int32_t lat,int32_t lon,int32_t alt,float eph,float epv,float v,float hdg)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 37, 149);
}
						           ...
						           uint64_t usec,uint8_t fix_type,int32_t lat,int32_t lon,int32_t alt,float eph,float epv,float v,float hdg)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 37, 149);
}
