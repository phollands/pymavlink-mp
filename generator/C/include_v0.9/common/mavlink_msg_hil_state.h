						       ...
						       uint64_t usec, float roll, float pitch, float yaw, float rollspeed, float pitchspeed, float yawspeed, int32_t lat, int32_t lon, int32_t alt, int16_t vx, int16_t vy, int16_t vz, int16_t xacc, int16_t yacc, int16_t zacc)
{
	...
	put_int32_t_by_index(lat, 32,  MAVLINK_PAYLOAD(msg)); // Latitude, expressed as * 1E7
	put_int32_t_by_index(lon, 36,  MAVLINK_PAYLOAD(msg)); // Longitude, expressed as * 1E7
	put_int32_t_by_index(alt, 40,  MAVLINK_PAYLOAD(msg)); // Altitude in meters, expressed as * 1000 (millimeters)
	put_int16_t_by_index(vx, 44,  MAVLINK_PAYLOAD(msg)); // Ground X Speed (Latitude), expressed as m/s * 100
	put_int16_t_by_index(vy, 46,  MAVLINK_PAYLOAD(msg)); // Ground Y Speed (Longitude), expressed as m/s * 100
	put_int16_t_by_index(vz, 48,  MAVLINK_PAYLOAD(msg)); // Ground Z Speed (Altitude), expressed as m/s * 100
	put_int16_t_by_index(xacc, 50,  MAVLINK_PAYLOAD(msg)); // X acceleration (mg)
	put_int16_t_by_index(yacc, 52,  MAVLINK_PAYLOAD(msg)); // Y acceleration (mg)
	put_int16_t_by_index(zacc, 54,  MAVLINK_PAYLOAD(msg)); // Z acceleration (mg)

	return mavlink_finalize_message(msg, system_id, component_id, 56, 180);
}
						           ...
						           uint64_t usec,float roll,float pitch,float yaw,float rollspeed,float pitchspeed,float yawspeed,int32_t lat,int32_t lon,int32_t alt,int16_t vx,int16_t vy,int16_t vz,int16_t xacc,int16_t yacc,int16_t zacc)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 56, 180);
}
						           ...
						           uint64_t usec,float roll,float pitch,float yaw,float rollspeed,float pitchspeed,float yawspeed,int32_t lat,int32_t lon,int32_t alt,int16_t vx,int16_t vy,int16_t vz,int16_t xacc,int16_t yacc,int16_t zacc)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 56, 180);
}
...
static inline void mavlink_msg_hil_state_send(mavlink_channel_t chan, uint64_t usec, float roll, float pitch, float yaw, float rollspeed, float pitchspeed, float yawspeed, int32_t lat, int32_t lon, int32_t alt, int16_t vx, int16_t vy, int16_t vz, int16_t xacc, int16_t yacc, int16_t zacc)
{
	...
	mavlink_msg_hil_state_pack_chan_send(chan, msg, usec, roll, pitch, yaw, rollspeed, pitchspeed, yawspeed, lat, lon, alt, vx, vy, vz, xacc, yacc, zacc);
}
