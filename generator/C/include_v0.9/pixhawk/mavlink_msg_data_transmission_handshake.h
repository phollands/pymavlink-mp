						       ...
						       uint8_t type, uint32_t size, uint8_t packets, uint8_t payload, uint8_t jpg_quality)
{
	...
	put_uint8_t_by_index(type, 0,  msg->payload); // type of requested/acknowledged data (as defined in ENUM DATA_TYPES in mavlink/include/mavlink_types.h)
	put_uint32_t_by_index(size, 1,  msg->payload); // total data size in bytes (set on ACK only)
