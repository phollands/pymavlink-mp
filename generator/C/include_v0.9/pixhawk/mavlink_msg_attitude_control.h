						       ...
						       uint8_t target, float roll, float pitch, float yaw, float thrust, uint8_t roll_manual, uint8_t pitch_manual, uint8_t yaw_manual, uint8_t thrust_manual)
{
	...
	put_uint8_t_by_index(target, 0,  msg->payload); // The system to be controlled
	put_float_by_index(roll, 1,  msg->payload); // roll
	put_float_by_index(pitch, 5,  msg->payload); // pitch
	put_float_by_index(yaw, 9,  msg->payload); // yaw
	put_float_by_index(thrust, 13,  msg->payload); // thrust
