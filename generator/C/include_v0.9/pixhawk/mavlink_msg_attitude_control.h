...
#define MAVLINK_MESSAGE_INFO_ATTITUDE_CONTROL { \
	"ATTITUDE_CONTROL", \
	9, \
	{  { "target", MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_attitude_control_t, target) }, \
         { "roll", MAVLINK_TYPE_FLOAT, 0, 1, offsetof(mavlink_attitude_control_t, roll) }, \
         { "pitch", MAVLINK_TYPE_FLOAT, 0, 5, offsetof(mavlink_attitude_control_t, pitch) }, \
         { "yaw", MAVLINK_TYPE_FLOAT, 0, 9, offsetof(mavlink_attitude_control_t, yaw) }, \
         { "thrust", MAVLINK_TYPE_FLOAT, 0, 13, offsetof(mavlink_attitude_control_t, thrust) }, \
						       ...
						       uint8_t target, float roll, float pitch, float yaw, float thrust, uint8_t roll_manual, uint8_t pitch_manual, uint8_t yaw_manual, uint8_t thrust_manual)
{
	...
	put_uint8_t_by_index(target, 0,  MAVLINK_PAYLOAD(msg)); // The system to be controlled
	put_float_by_index(roll, 1,  MAVLINK_PAYLOAD(msg)); // roll
	put_float_by_index(pitch, 5,  MAVLINK_PAYLOAD(msg)); // pitch
	put_float_by_index(yaw, 9,  MAVLINK_PAYLOAD(msg)); // yaw
	put_float_by_index(thrust, 13,  MAVLINK_PAYLOAD(msg)); // thrust
	...
	return mavlink_finalize_message(msg, system_id, component_id, 21, 225);
}
						           ...
						           uint8_t target,float roll,float pitch,float yaw,float thrust,uint8_t roll_manual,uint8_t pitch_manual,uint8_t yaw_manual,uint8_t thrust_manual)
{
	...
	return mavlink_finalize_message_chan(msg, system_id, component_id, chan, 21, 225);
}
						           ...
						           uint8_t target,float roll,float pitch,float yaw,float thrust,uint8_t roll_manual,uint8_t pitch_manual,uint8_t yaw_manual,uint8_t thrust_manual)
{
	...
	mavlink_finalize_message_chan_send(msg, chan, 21, 225);
}
