						       ...
						       float x, float y, float z, uint8_t orientation_assignment, uint16_t size, uint16_t orientation, const uint8_t descriptor[32], float response)
{
	...
	put_uint8_t_by_index(orientation_assignment, 12,  msg->payload); // Orientation assignment 0: false, 1:true
	put_uint16_t_by_index(size, 13,  msg->payload); // Size in pixels
	put_uint16_t_by_index(orientation, 15,  msg->payload); // Orientation
	put_uint8_t_array_by_index(descriptor, 17, 32,  msg->payload); // Descriptor
	put_float_by_index(response, 49,  msg->payload); // Harris operator response at this location
...
{
	return MAVLINK_MSG_RETURN_uint16_t(msg,  13);
}
...
{
	return MAVLINK_MSG_RETURN_uint16_t(msg,  15);
}
...
{
	return MAVLINK_MSG_RETURN_uint8_t_array(msg, descriptor, 32,  17);
}
...
{
	return MAVLINK_MSG_RETURN_float(msg,  49);
}
